<template>
  ...
  >
    ...
    >
      <div class="divcol center" style="gap: 5px">
        <img
          ...
          "
        />
        <span class="h11_em bold"
          >{{ userDc.username }}#{{ userDc.discriminator }}</span
        >
      </div>

      <v-card class="card" style="--bg: hsl(0 0% 60% / 0.4)">
    ...
    <v-icon
      v-else-if="resultOperation === 'success'"
      size="7em"
    ...
    >
    <v-icon
      v-else-if="resultOperation === 'cancel'"
      size="7em"
...
export default {
  ...
  methods: {
    ...
    connectDiscord() {
      ...
      this.$axios
        .post(`${this.baseUrl}api/v1/save-user-discord/`, {
          wallet: accountId,
          discord_id: this.userDc.id,
        })
        .then((result) => {
          this.$axios
            .post(process.env.BOTDISCORD_URL + '/api/v1/active-rol', {
              wallet: accountId,
            })
            .then((result) => {
