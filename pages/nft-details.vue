    ...
    <section class="header grid">
          ...
          >
            <template #message>
              <span class="media-label">{{
                $refs.track?.currentTime.formatTime()
              }}</span>
              <span class="media-label">{{
                $refs.track?.duration.formatTime()
              }}</span>
            </template>
          </v-slider>
        </div>
      ...
      <article class="card divcol" style="gap: 30px">
        ...
        <div class="spacea">
          ...
          <div class="divcol aend" style="gap: 0.5em">
            <span class="bold" style="--c: var(--accent)"
              >$ {{ Number(nft_main.price)?.toFixed(2) }}
            </span>
            <span style="font-size: calc(var(--font-text) / 1.2)"
              ><img
                src="~/assets/sources/logos/near.svg"
                alt="near"
                style="--w: 0.75em"
              />
              {{ nft_main.price_near }}</span
            >
            <span style="font-size: calc(var(--font-text) / 1.2)"
              >Storage Deposit:
              <img
        ...
        <div class="center gap2 wrap bold">
          ...
          <v-btn
            ...
            :ripple="false"
            class="btn activeBtn"
            style="--w: min(100%, 12em); --fs: 14px"
            @click="openBuyModal"
            >{{ this.isVip ? 'just mint' : 'buy' }}</v-btn
          >
          <v-btn
            v-if="soldBtn"
            :disabled="true"
            :ripple="false"
            class="btn activeBtn text-decoration-line-through"
            style="--w: min(100%, 12em); --fs: 14px"
            >Sold Out</v-btn
          >
        </div>
        ...
        <div class="spacea">
          <h10 style="color: #d57535"
            >Warning: If the window does not open, you must enable pop-up
            windows in your browser.</h10
          >
        </div>
    ...
    <v-expansion-panels class="custome-expansion mt-10" :value="0">
      <v-expansion-panel>
        <v-expansion-panel-header expand-icon="mdi-menu-down" class="bold"
          >Buy Resale</v-expansion-panel-header
        >
          ...
          >
            ...
            <template #[`item.seller`]="{ item }">
              <center class="center" style="gap: 10px">
                ...
                <span :title="item.seller">{{
                  item.seller.limitString(25)
                }}</span>
              </center>
            </template>
            ...
            <template #[`item.buy`]="{ item }">
              <v-btn
                ...
                style="--min-w: 112px; --w: min(100%, 8em); --fs: 14px"
                @click="buyMarketRamper(item)"
                >Buy</v-btn
              >
              <v-btn
                v-if="item.owned"
...
export default {
  ...
  async mounted() {
    ...
    if (this.$ramper.getUser() && this.nft_main.tier === 1) {
      ...
      this.isVip = await contract.is_vip({
    ...
    buyNftFiat() {
      window.open(
        'https://checkout.ramper.xyz/buy?contract_address=' +
          ...
          '&user_wallet_address=' +
          (this.$ramper.getAccountId() === null
            ? ''
            : this.$ramper.getAccountId()) +
          '&token_series_id=' +
          this.nft_main.token_id
      )
    },
    ...
    async getSerie() {
      ...
      if (res.data.series[0]) {
        ...
        if (
          ...
          Number(data.supply) >= Number(data.copies)
        ) {
          this.soldBtn = true
        }
